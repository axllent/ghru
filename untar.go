...
func extractArchive(filePath string, directory string) error {
	...
	for {
		...
		// Check for TarSlip vulnerability: Ensure the file path is within the destination directory.
		if !strings.HasPrefix(filename, filepath.Clean(directory)+string(os.PathSeparator)) {
